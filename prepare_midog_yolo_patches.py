...
OUTPUT_DIR = "datasets/midog_yolo_patches"
...
def create_patches(img_array: np.ndarray, patch_size: int = 480, overlap: int = 0) -> Tuple[List[np.ndarray], List[Tuple[int, int, int, int]]]:
...
print("Processing images and creating patches...")
patch_counter = 0
    ...
    try:
        ...
        for patch_idx, (patch, (x_start, y_start, x_end, y_end)) in enumerate(zip(patches, patch_coords)):
            ...
            with open(label_path, 'w') as f:
                ...
                # If no valid annotations, create empty file (no dummy annotation)
                if not valid_annotations:
                    pass  # Empty file is valid for YOLO
        
    except Exception as e:
        print(f"Error processing {tiff_file}: {str(e)}")

# Clean up old cache files if they exist
for cache_file in ['datasets/midog_yolo_patches/train/labels.cache', 'datasets/midog_yolo_patches/val/labels.cache']:
    if os.path.exists(cache_file):
        os.remove(cache_file)
        print(f"Removed old cache file: {cache_file}")

# Create dataset.yaml file
yaml_content = f"""# MIDOG Patches dataset
train: train/images
val: val/images
...
print("Patch dataset preparation complete!")
